...
service / on defaultListener {
    resource function get validation(@http:Header {name: "tracking-id"} string? trackingIdHeader, int intId) returns Validation|http:BadRequest {
        if trackingIdHeader is () {
            return <http:BadRequest>{body: "'tracking-id' header is required"};
        }
